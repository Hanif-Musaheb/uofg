...
;    if finished then goto CommandLoopDone
;    CurrentCode := (*InputPtr).code
;    p := list[(*InputPtr).i]     ; linked list
;    x := (*InputPtr).x      ; value to insert, delete, search
;    if CurrentCode < 0 then goto CommandCodeElse
...
; Initialize the command loop
    ...
    store  R0,finished[R0]     ; finished := 0

CommandLoop
    load   R5,InputPtr[R0]     ; R1 := InputPtr
    lea    R6,InputEnd[R0]     ; R6 := &InputEnd
    cmp    R5,R6               ; compare InputPtr with InputEnd
    jumpge CommandLoopDone[R0] ; if InputPtr >= &InputEnd then goto CommandLoopDone
    load   R10,finished[R0]    ; R10 := finished
    cmp    R10,R0              ; compare finished with 0
    jumpne CommandLoopDone[R0] ; if finished then goto CommandLoopDone
...
CmdTerminate
    ...
    lea    R10,1[R0]           ; R10 := 1
    store  R10,finished[R0]    ; finished := 1
    jump   CommandLoop[R0]     ; goto CommandLoop
